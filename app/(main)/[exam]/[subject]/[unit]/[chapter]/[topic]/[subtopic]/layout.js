...
export async function generateMetadata({ params }) {
  const { exam: examSlug, subject: subjectSlug, unit: unitSlug, chapter: chapterSlug, topic: topicSlug, subtopic: subtopicSlug } = await params;
    ...
    try {
      ...
      try {
        const { fetchExamById, fetchSubjectById, fetchUnitById, fetchChapterById, fetchTopicById, fetchSubTopicById, fetchSubTopicDetailsById } = await import("../../../../../../lib/api");
    ...
    const seoData = {
      title: subtopicDetails?.title || (subtopic.name && subject?.name && exam?.name ? `${subtopic.name} - ${subject.name} - ${exam.name}` : subtopic.name || "SubTopic"),
      metaDescription:
        subtopicDetails?.metaDescription ||
      ...
      keywords: subtopicDetails?.keywords || (subtopic.name && subject?.name && exam?.name ? `${subtopic.name}, ${subject.name}, ${exam.name}` : subtopic.name || "SubTopic"),
    };
