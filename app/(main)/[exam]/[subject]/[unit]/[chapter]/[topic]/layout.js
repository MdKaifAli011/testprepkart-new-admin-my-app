...
export async function generateMetadata({ params }) {
  ...
  try {
    ...
    try {
      const { fetchExamById, fetchSubjectById, fetchUnitById, fetchChapterById, fetchTopicById } = await import("../../../../../lib/api");
    ...
    const seoData = {
      title: topic.title || (topic.name && subject?.name && exam?.name ? `${topic.name} - ${subject.name} - ${exam.name}` : topic.name || "Topic"),
      metaDescription:
      ...
      keywords: topic.keywords || (topic.name && subject?.name && exam?.name ? `${topic.name}, ${subject.name}, ${exam.name}` : topic.name || "Topic"),
    };
