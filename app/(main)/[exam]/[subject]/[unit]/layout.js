...
export async function generateMetadata({ params }) {
  ...
  try {
    ...
    try {
      const { fetchExamById, fetchSubjectById, fetchUnitById } = await import("../../../lib/api");
    ...
    const seoData = {
      title: unit.title || (unit.name && subject?.name && exam?.name ? `${unit.name} - ${subject.name} - ${exam.name}` : unit.name || "Unit"),
      metaDescription:
      ...
      keywords: unit.keywords || (unit.name && subject?.name && exam?.name ? `${unit.name}, ${subject.name}, ${exam.name}` : unit.name || "Unit"),
    };
