...
const UnitPage = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const prev = await getPreviousUnit({
          examId: examIdValue,
          examSlug: createSlug(fetchedExam.name),
          subjectId: foundSubject._id,
          subjectSlug: createSlug(foundSubject.name),
        ...
        if (!isCancelled) {
          logger.error("Error loading unit data:", err);
  ...
  if (error || !exam || !subject || !unit) {
    return <ErrorState message={error || ERROR_MESSAGES.UNIT_NOT_FOUND} />;
  }
